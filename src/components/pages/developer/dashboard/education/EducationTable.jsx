...
const EducationTable = ({isLoading, isFetching, education, setItemEdit}) => {
  ...
  return (
    ...
    <div className="table-wrapper relative">
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th className='w-[80px]'>School</th>
                        <th className='w-[80px]'>Date Started to End</th>
        ...
        {education?.data.length === 0 && (
            ...
            {education?.data.map((item, key)=>(
                ...
                <td>{item.education_publish_date}</td>
                <td className='table-action'>
                    <ul>
                        {item.education_is_active ? (
                            <>
    ...
    {store.isActive && <ModalConfirm position="center" queryKey="education" endpoint={`/v1/education/active/${id}`} 
        isArchiving={isArchiving}/>}

    {store.isDelete && <ModalDelete position="center" endpoint={`/v1/education/${id}`} 
    queryKey="education"/>}
